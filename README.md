...
메뉴판의 질감을 따릅니다. **그라데이션·광택·네온 발광은 쓰지 않습니다.**
거친 종이결(`--grain`) 위에 2px 잉크 선, 매트하게 칠한 면, 손으로 그린 듯
미세하게 어긋난 모서리(`--hand-1/2/3`)로 통일했습니다.

- 본문 Pretendard, 제목 도현체(Do Hyeon), 주문번호 Creepster
...
고객 화면 상단 제목·배경은 별도로 제작한 그림을 넣습니다.

- `assets/brand-header.png` 로 저장하면 자동으로 표시됩니다.
  파일이 없으면 점선 자리 표시 상자가 보입니다.
- 페이지 전체 배경 그림은 `assets/styles.css` 맨 위 주석 블록의 주석을 풀고
  파일명을 맞추면 됩니다.
